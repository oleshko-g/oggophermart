...
import (
	...
	balancesvr "github.com/oleshko-g/oggophermart/internal/gen/http/balance/server"
	usersvr "github.com/oleshko-g/oggophermart/internal/gen/http/user/server"
...
func handleHTTPServer(ctx context.Context,
	u *url.URL, balanceEndpoints *balance.Endpoints, userEndpoints *user.Endpoints, wg *sync.WaitGroup, errc chan error, dbg bool) {
	...
	// responses.
	var (
		balanceServer *balancesvr.Server
		userServer    *usersvr.Server
	)
	{
		eh := errorHandler(ctx)
		balanceServer = balancesvr.New(balanceEndpoints, mux, dec, enc, eh, nil)
	...
	for _, m := range balanceServer.Mounts {
